...
Pod::Spec.new do |s|
  ...
  s.source       = { :http => "http://sourceforge.net/projects/boost/files/boost/1.53.0/boost_1_53_0.tar.gz",
                     :sha1 => "0e4ef26cc7780c6bbc63987ef2f29be920e2395b" }

  s.requires_arc = false
  s.xcconfig = { "HEADER_SEARCH_PATHS" => "${PODS_ROOT}/boost" }

  s.subspec 'string_algorithms-includes' do |sp|
    sp.preserve_paths = [
  ...
  s.subspec "shared_ptr-includes" do |sp|
    sp.preserve_paths = [
  ...
  s.subspec "pointer_cast-includes" do |sp|
    sp.preserve_paths = [
      "boost/pointer_cast.hpp"
    ]
  end
  s.subspec "numeric-includes" do |sp|
    sp.preserve_paths = [
      "boost/numeric/**/*.hpp"
    ]
  end
  s.subspec "preprocessor-includes" do |sp|
    sp.preserve_paths = [
      "boost/preprocessor/**/*.hpp"
    ]
  end
  s.subspec "math-includes" do |sp|
    sp.preserve_paths = [
      "boost/math/**/*.hpp",
      "boost/limits.hpp",
      "boost/static_assert.hpp",
  ...
  s.subspec "graph-includes" do |sp|
    sp.preserve_paths = [
      "boost/*.hpp",
